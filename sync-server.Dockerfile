...
# crdt is a workspace dep that yarn linked as `node_modules/@actual-app/crdt -> ../../packages/crdt`,
# which becomes a dangling symlink in the final stage since /app/packages isn't copied.
...
# Same treatment for @actual-app/crdt — copy its package.json and built dist out of the workspace
# so the final image can resolve it without /app/packages.

...
This repo packages [Actual Budget](https://github.com/actualbudget/actual) for StartOS.
...
Actual Budget is built from a git submodule at `actual/`, using `actual/sync-server.Dockerfile` from upstream. There is no pinned dockerTag in the manifest — the image is built fresh from whatever commit the submodule points at.
